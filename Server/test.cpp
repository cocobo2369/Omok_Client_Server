...
vector<string> getTokens(string input, char delimiter) {
	...
	istringstream f(input); //토크나이징하려고
	string s;
...
void ServerThread(Client* client) {
	char* sent = new char[256]; //server가 보낼 거
	char* received = new char[256]; //client가 보낸 거
	int size = 0;
	while (true) {
		ZeroMemory(received, 256); //memset(receive,0, 256) 과 같음 단지 0으로 memset만 할 수 있는 함수라고 생각하면됨
